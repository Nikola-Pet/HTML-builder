...
interface NewsletterCardProps {
  ...
  keywords?: string[];
  imageHtml?: string;
  isDraft?: boolean; // Flag to indicate if this is a draft
}
...
export const NewsletterCard: React.FC<NewsletterCardProps> = ({
  ...
  const handleCardClick = () => {
    // Navigate to the builder and pass the appropriate ID
    if (id) {
      if (isDraft) {
        navigate("/builder", { state: { draftId: id } });
      } else {
        navigate("/builder", { state: { newsletterId: id } });
      }
    } else {
      navigate("/builder");
    }
  ...
  return (
    <Card
      ...
      style={{ border: "1px solid var(--bosch-gray-90)" }}
    >
      <CardHeader
        className="flex flex-row items-center justify-between px-4 py-0 space-y-0 gap-2"
        style={{
          alignItems: "center",
          backgroundColor: "var(--bosch-gray-85)",
        }}
      >
        <CardTitle className="text-lg font-bold leading-none flex items-center">
